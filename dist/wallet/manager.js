...
export class WalletManager {
    static async getModules() {
        ...
        const { createKeystore, InMemoryTransactionHistoryStorage, PublicKey, UnshieldedWallet, } = await importProjectModule(projectRoot, 'wallet-sdk-unshielded-wallet');
    ...
    static async create(seed, networkConfig) {
        const modules = await this.getModules();
        const { ledger, WalletFacade, DustWallet, Roles, ShieldedWallet, createKeystore, InMemoryTransactionHistoryStorage, PublicKey, UnshieldedWallet, getNetworkId, } = modules;
    ...
    static async waitForSync(wallet) {
        await Rx.firstValueFrom(wallet.state().pipe(Rx.throttleTime(5000), Rx.filter((s) => s.isSynced)));
    }
    static async waitForFunds(wallet) {
        const { ledger } = await this.getModules();
        Logger.info('Waiting for funds...');
        await Rx.firstValueFrom(wallet.state().pipe(Rx.throttleTime(10000), Rx.filter((s) => s.isSynced), Rx.map((s) => s.unshielded.balances[ledger.unshieldedToken().raw] ?? 0n), Rx.filter((b) => b > 0n)));
        Logger.success('Funds received!');
    }
    static async ensureDust(walletCtx) {
