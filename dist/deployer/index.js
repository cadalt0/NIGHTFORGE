...
export class Deployer {
    static async deploy(contractName, options) {
        ...
        try {
            ...
            let proofServerStatus;
            try {
                proofServerStatus = FileSystem.readJSON(proofServerStatusPath);
            }
            catch (error) {
            ...
            if (!FileSystem.exists(path.join(zkConfigPath, 'contract', 'index.js'))) {
                throw new Error(`Contract "${contractName}" not compiled. Run: npx nightforge compile`);
            }
            Logger.section('Step 1: Loading Contract');
            const contractPath = path.join(zkConfigPath, 'contract', 'index.js');
            const contractModule = await import(pathToFileURL(contractPath).href);
            const compiledContract = CompiledContract.make(contractName, contractModule.Contract).pipe(CompiledContract.withVacantWitnesses, CompiledContract.withCompiledFileAssets(zkConfigPath));
            ...
            if (!seed) {
                ...
                if (selectedWallet?.seed) {
                    ...
                    if (selectedWallet.network && selectedWallet.network !== options.network) {
                        Logger.warn(`Wallet "${selectedWallet.name}" network is "${selectedWallet.network}" but deploy network is "${options.network}"`);
                    }
                }
            }
            if (!seed) {
                throw new Error('No private key provided. Use --private-key, set MIDNIGHT_PRIVATE_KEY, create a wallet with "npx nightforge wallet create", or select one with --wallet.');
            }
            ...
            if (balance === 0n) {
                ...
                await withSpinner('Waiting for funds...', async () => WalletManager.waitForFunds(walletCtx.wallet));
            }
            // Ensure DUST
            Logger.section('Step 3: DUST Token Setup');
            await withSpinner('Ensuring DUST tokens...', async () => WalletManager.ensureDust(walletCtx));
            // Deploy
            Logger.section('Step 4: Deploying Contract');
            const providers = await withSpinner('Setting up providers...', async () => ProviderFactory.create(walletCtx, networkConfig, zkConfigPath, `${contractName}-state`));
            const deployed = await withSpinner('Deploying contract (this may take 30-60s)...', async () => deployContract(providers, {
