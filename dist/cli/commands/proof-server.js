...
function startProofServer(version, port) {
    ...
    try {
        ...
        dockerCheck.on('close', (code) => {
            ...
            proofServerProcess = spawn('docker', [
                'run',
                '--rm',
                '-p',
                `${port}:6300`,
            ...
            const writeStatus = (running, extra = {}) => {
                const status = {
                    running,
                    port,
            ...
            proofServerProcess.stderr?.on('data', (chunk) => {
                const text = chunk.toString();
...
//# sourceMappingURL=proof-server.js.map